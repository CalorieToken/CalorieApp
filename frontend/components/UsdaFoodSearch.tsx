...
export function UsdaFoodSearch({ locale, disabled, canLog = true, onChoose, onEditing, confirmation, feedback, requestedIngredient }: {
  locale: string;
  disabled: boolean;
  canLog?: boolean;
  ...
  feedback?: ReactNode;
  requestedIngredient?: { fdcId: number; grams: number; serial: number };
}) {
  ...
  const [selectedSection, setSelectedSection] = useState("nutrition");
  const [alternativePage, setAlternativePage] = useState(0);
  const [history, setHistory] = useState<{ food: UsdaFood; amount: string; amountLocale: string; section: string; scrollTop: number }[]>([]);
  const handledIngredient = useRef<number>();
  const chosenIngredient = useRef<number>();
  ...
  const alternatives = useMemo(() => selected && catalogue ? similarFoodNames(selected, catalogue.foods,
    food => food.description, food => String(food.fdc_id), 24) : [], [selected, catalogue]);
  ...
  async function search(event: FormEvent<HTMLFormElement> | null, term = query) {
    event?.preventDefault();
    if (disabled || state === "loading" || !term.trim()) return;
    onEditing(); setSelected(null); setAnchorId(null); setHistory([]); setLimit(6); setSubmitted(term.trim());
    if (catalogue) return;
  ...
  function choose(food: UsdaFood, alternative = false) {
    ...
    if (!selected || !alternative) {
      ...
      setHistory(previous => [...previous, { food: selected, amount, amountLocale, section: selectedSection, scrollTop: resultsList.current?.scrollTop ?? 0 }]);
    }
    setSelectedSection("nutrition"); setAlternativePage(0);
  ...
  function previousFood() {
    ...
    setSelectedSection(previous.section); setAlternativePage(0);
  ...
  const receiveIngredient = useRef<(request: NonNullable<typeof requestedIngredient>) => void>(() => {});
  receiveIngredient.current = requested => {
    setQuery(String(requested.fdcId));
    void search(null, String(requested.fdcId));
  };
  useEffect(() => {
    if (!requestedIngredient || disabled || state === "loading" || handledIngredient.current === requestedIngredient.serial) return;
    handledIngredient.current = requestedIngredient.serial;
    receiveIngredient.current(requestedIngredient);
  }, [requestedIngredient, disabled, state]);
  const selectIngredient = useRef<(request: NonNullable<typeof requestedIngredient>) => void>(() => {});
  selectIngredient.current = requested => {
    const food = catalogue?.foods.find(candidate => candidate.fdc_id === requested.fdcId);
    if (!food) return;
    choose(food); setAmount(String(requested.grams)); setAmountLocale("en");
  };
  useEffect(() => {
    if (!requestedIngredient || !catalogue || disabled || submitted !== String(requestedIngredient.fdcId) || chosenIngredient.current === requestedIngredient.serial) return;
    chosenIngredient.current = requestedIngredient.serial;
    selectIngredient.current(requestedIngredient);
  }, [requestedIngredient, catalogue, disabled, submitted]);
  ...
  const selectedCard = selected && values ? <div ref={detail} data-testid="usda-selected-food" tabIndex={-1} className="mt-5 min-w-0 rounded-xl border-2 border-brand-secondary/30 bg-brand-bg p-4 focus-visible:ring-2 focus-visible:ring-brand-secondary">
      ...
      <FoodExplore key={selected.fdc_id} locale={locale} productName={selected.description} currentSection={selectedSection} onSectionChange={setSelectedSection}
        onBack={disabled ? undefined : changeFood} backLabel={ui.copy.changeFood}
        nutrition={      <><dl data-testid="usda-nutrition-preview" className="mt-4 grid grid-cols-2 gap-3">{([['calories', copy.calories, 'kcal'], ['protein', copy.protein, 'g'], ['fat', copy.fat, 'g'], ['carbohydrates', copy.carbohydrates, 'g']] as const).map(([key, label, unit]) => <div key={key} className="min-w-0 break-words">
      ...
      <p className="mt-3 text-sm text-brand-secondary">{ui.copy.gradeUnavailable}</p>
        ...
        labels={<><p className="text-sm text-brand-secondary">{copy.usdaIntro}</p>
          <a href={`https://fdc.nal.usda.gov/food-details/${selected.fdc_id}/nutrients`} target="_blank" rel="noopener noreferrer"
          className="mt-2 block min-h-11 break-words py-2 text-sm font-semibold text-brand-secondary underline">{copy.source} · <bdi>FDC {selected.fdc_id}</bdi> ↗</a></>}
        comparison={alternatives.length ? <div>
          <p className="text-sm font-semibold text-brand-primary">{copy.similarTo.replace("{food}", selected.description)}</p>
          <ul className="mt-2 grid gap-2 sm:grid-cols-2">{alternatives.slice(alternativePage * 2, alternativePage * 2 + 2).map(food => <li key={food.fdc_id}><button type="button" disabled={disabled} onClick={() => choose(food, true)}
          ...
          {alternatives.length > 2 ? <div className="mt-2 flex items-center justify-center gap-3">
            <button type="button" disabled={alternativePage === 0} aria-label={copy.previous} onClick={() => setAlternativePage(value => value - 1)} className="min-h-11 min-w-11 rounded-lg border border-brand-secondary/30 disabled:opacity-40">←</button>
            <span className="text-sm">{alternativePage * 2 + 1}–{Math.min(alternativePage * 2 + 2, alternatives.length)} / {alternatives.length}</span>
            <button type="button" disabled={(alternativePage + 1) * 2 >= alternatives.length} aria-label={copy.next} onClick={() => setAlternativePage(value => value + 1)} className="min-h-11 min-w-11 rounded-lg border border-brand-secondary/30 disabled:opacity-40">→</button>
          </div> : null}
          <p className="mt-2 text-xs text-brand-secondary">{copy.suggestionNote}</p>
        </div> : undefined}
        recipes={logItem ? <FoodRecipeIdeas key={selected.fdc_id} food={logItem} locale={locale} embedded disabled={disabled}
          onLog={canLog ? onChoose : undefined}
          onIngredient={(fdcId, grams) => { const ingredient = catalogue?.foods.find(food => food.fdc_id === fdcId); if (ingredient && !disabled) { choose(ingredient, true); setAmount(String(grams)); setAmountLocale("en"); } }} /> : undefined} />
    </div> : feedback;
