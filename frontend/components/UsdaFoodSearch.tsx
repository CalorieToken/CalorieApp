...
export function UsdaFoodSearch({ locale, disabled, onChoose, onEditing, confirmation, feedback }: {
  ...
  return <details id="calorie-basic-foods" tabIndex={-1} className="min-w-0 rounded-xl border border-brand-secondary/20 bg-white p-4 sm:p-5" lang={ui.locale} dir={ui.direction}>
    ...
    {selected && values ? <div ref={detail} tabIndex={-1} className="mt-5 min-w-0 rounded-xl border-2 border-brand-secondary/30 bg-brand-bg p-4 focus-visible:ring-2 focus-visible:ring-brand-secondary">
      ...
      {!confirmation ? <dl className="mt-4 grid grid-cols-2 gap-3">{([['calories', copy.calories, 'kcal'], ['protein', copy.protein, 'g'], ['fat', copy.fat, 'g'], ['carbohydrates', copy.carbohydrates, 'g']] as const).map(([key, label, unit]) => <div key={key} className="min-w-0 break-words">
