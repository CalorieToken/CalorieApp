...
export function TestnetEntry({ onNavigate, onOpenAccountTools }: TestnetEntryProps) {
  ...
  return (
    <section className="calorie-journey-card rounded-2xl border border-brand-secondary/20 bg-white p-4 shadow-sm sm:p-6"
      ...
      <div id="account-journey-panel-test" role="tabpanel" aria-labelledby="account-journey-tab-test"
        ...
        <a href="https://calorietoken.net/index.php/calorieapp/#ctstyle-testnet" target="_top" onClick={openGuide}
      ...
      <div className="mt-6 flex items-center justify-between gap-3 border-t border-brand-secondary/10 pt-4">
        <button type="button" onClick={() => selectStep(steps[Math.max(0, stepIndex - 1)], true)} disabled={stepIndex === 0}
          className="min-h-11 rounded-full border-2 border-brand-secondary px-4 py-2 text-sm font-bold text-brand-secondary disabled:invisible">{copy.previous}</button>
        <button type="button" onClick={() => selectStep(steps[Math.min(steps.length - 1, stepIndex + 1)], true)} disabled={stepIndex === steps.length - 1}
          className="min-h-11 rounded-full bg-brand-primary px-4 py-2 text-sm font-bold text-white disabled:invisible">{copy.next}</button>
      </div>
    </section>
  );
}
