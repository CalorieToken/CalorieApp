...
import foodTranslations from "@/config/calorieverse-food-copy.json";
import { foodActivity, foodActivityStatus, reviewFoodSample } from "@/lib/calorieVerseActivities";
import {
...
type SceneObject = MeadowObject | typeof foodActivity.object;
const sceneObjects: SceneObject[] = [...meadow.objects, foodActivity.object];
...
export function CalorieVerseMeadow() {
  ...
  const foodCopy = (foodTranslations as Record<string, typeof foodTranslations.en>)[locale] ?? foodTranslations.en;
  ...
  const foodStatus = foodActivityStatus(state);
  ...
  function perform(object: SceneObject) {
  ...
  function nudge(direction: Point) {
    target.current = null;
    const next = {...current.current, position: position({
      x: current.current.position.x + direction.x * 2.4,
      y: current.current.position.y + direction.y * 2.4,
    })};
    current.current = next;
    setState(next);
  }
  function reviewFood(answerId: string) {
    if (paused || !loaded) return;
    const updated = reviewFoodSample(current.current, answerId);
    if (updated !== current.current) {
      current.current = updated;
      setState(updated);
      setFoodFeedback(false);
    } else if (nearby(current.current.position, foodActivity.object) && foodActivityStatus(current.current) === "new") {
      setFoodFeedback(true);
    }
  }
  function label(object: SceneObject): string {
    if (object.kind === "food-data") return foodCopy.stand;
  ...
  return <main className={styles.page} data-calorieapp-content lang={locale} dir={localeDirection(locale)}>
    ...
    <div className={styles.layout}>
      <section className={styles.worldPanel} aria-labelledby="meadow-title">
        ...
        <div ref={stage} className={styles.stage} tabIndex={0} role="group" aria-label={copy.place+". "+copy.controls}
          onKeyDown={event => {
            ...
            if (direction) {event.preventDefault();if(!event.repeat)nudge(direction);held.current=direction;}
            if (event.key.toLowerCase() === "e") {event.preventDefault(); if (!event.repeat) {
              const close = sceneObjects.filter(object=>nearby(current.current.position,object) && !(object.kind==="apple" && current.current.collected.includes(object.id))).sort((a,b)=>Math.hypot(a.x-current.current.position.x,a.y-current.current.position.y)-Math.hypot(b.x-current.current.position.x,b.y-current.current.position.y))[0];
          ...
          {sceneObjects.map(object => {
            ...
            return <button type="button" key={object.id} className={[styles.object, styles[object.kind],collected?styles.collected:"",selectedId===object.id?styles.selected:""].join(" ")}
              ...
              {object.kind==="food-data" ? <span className={styles.foodStand} aria-hidden="true"><Image src={foodActivity.sample.image} width={36} height={36} alt="" /><span>✓</span></span> : object.kind==="apple" ? <Image src="/images/food-illustrations/apple.svg" width={42} height={42} alt="" /> : object.kind==="garden" ? <span className={styles.bed}>{state.garden==="empty"?"· · ·":state.garden==="planted"?"🌱":state.garden==="watered"?<Image src="/images/food-illustrations/carrot.svg" width={55} height={55} alt="" />:"✓"}</span> : <span className={styles.guideIcon}>C<span>••</span></span>}
              <span className={styles.objectLabel}>{object.kind==="food-data"?foodCopy.stand:object.kind==="garden"?copy.garden:object.kind==="guide"?copy.guide:collected?"✓":""}</span>
            </button>;
          })}
          ...
          <div className={styles.pad} dir="ltr">
            {directionButtons.map(([direction,vector])=><button type="button" key={direction} className={styles[direction]} aria-label={copy[direction]} disabled={paused || !loaded}
              onPointerDown={event=>{event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);const direction={x:vector[0],y:vector[1]};nudge(direction);held.current=direction;}}
              onPointerUp={()=>{held.current=null;}} onPointerCancel={()=>{held.current=null;}}
              onClick={event=>{if(event.detail===0) nudge({x:vector[0],y:vector[1]});}}>
      ...
      <aside className={styles.side}>
        ...
        <section className={styles.actionCard} aria-labelledby="meadow-action-title">
          <span className={styles.eyebrow}>{selected.kind==="guide"?copy.guide:copy.place}</span><h2 id="meadow-action-title">{label(selected)}</h2>
          {selected.kind === "food-data" ? <>
            <p>{foodCopy.intro}</p>
            {foodStatus === "unsupported" ? <p role="status">{foodCopy.unsupported}</p> : foodStatus === "complete" ? <p role="status">✓ {foodCopy.complete}</p> : nearby(state.position, selected) ? <>
              <p id="food-question">{foodCopy.question}</p>
              <Image src={foodActivity.sample.image} width={90} height={90} alt={foodCopy.apple} />
              <div className={styles.foodChoices} role="group" aria-labelledby="food-question">
                {foodActivity.sample.choices.map(id => <button type="button" key={id} disabled={paused || !loaded} onClick={()=>reviewFood(id)}>{foodCopy[id as "apple" | "carrot"]}</button>)}
              </div>
              <p role="status" aria-live="polite">{foodFeedback ? foodCopy.tryAgain : ""}</p>
            </> : <button type="button" disabled={paused || !loaded} onClick={()=>perform(selected)}>{copy.walk}</button>}
            <p className={styles.sampleNote}>{foodCopy.sampleNote}</p>
            <Link href={`/?ui_lang=${encodeURIComponent(locale)}`}>{foodCopy.openApp} ↗</Link>
          </> : <>
