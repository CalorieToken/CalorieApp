...
(function () {
  ...
  function safeQueryKey(key) {
    return key === "ui_lang" || /^(?:utm_(?:source|medium|campaign|term|content|id)|gclid|dclid|fbclid|msclkid|ttclid|twclid)$/.test(key);
  }
  ...
  function refineHowToBuy(id) {
    ...
    if (!["https://calorietoken.net", "https://www.calorietoken.net"].includes(url.origin)
      ...
      || !Array.from(url.searchParams.keys()).every(safeQueryKey)) return;
  ...
  function startRichlists() {
    ...
    var pagerCopy = {
      en:{search:"Filter by rank or address",previous:"Previous",next:"Next",status:"{from}–{to} of {total}",empty:"No holders found"},
      nl:{search:"Filter op rang of adres",previous:"Vorige",next:"Volgende",status:"{from}–{to} van {total}",empty:"Geen houders gevonden"},
      "zh-Hans":{search:"按排名或地址筛选",previous:"上一页",next:"下一页",status:"第 {from}–{to} 项，共 {total} 项",empty:"未找到持有者"},
      hi:{search:"रैंक या पते से फ़िल्टर करें",previous:"पिछला",next:"अगला",status:"{total} में से {from}–{to}",empty:"कोई धारक नहीं मिला"},
      es:{search:"Filtrar por rango o dirección",previous:"Anterior",next:"Siguiente",status:"{from}–{to} de {total}",empty:"No se encontraron titulares"},
      ar:{search:"تصفية حسب الترتيب أو العنوان",previous:"السابق",next:"التالي",status:"{from}–{to} من {total}",empty:"لم يتم العثور على حائزين"},
      fr:{search:"Filtrer par rang ou adresse",previous:"Précédent",next:"Suivant",status:"{from}–{to} sur {total}",empty:"Aucun détenteur trouvé"},
      bn:{search:"র‌্যাঙ্ক বা ঠিকানা দিয়ে ফিল্টার করুন",previous:"আগের",next:"পরের",status:"{total}-এর মধ্যে {from}–{to}",empty:"কোনো ধারক পাওয়া যায়নি"},
      pt:{search:"Filtrar por posição ou endereço",previous:"Anterior",next:"Seguinte",status:"{from}–{to} de {total}",empty:"Nenhum titular encontrado"},
      id:{search:"Filter berdasarkan peringkat atau alamat",previous:"Sebelumnya",next:"Berikutnya",status:"{from}–{to} dari {total}",empty:"Pemegang tidak ditemukan"},
      ur:{search:"درجہ یا پتے سے فلٹر کریں",previous:"پچھلا",next:"اگلا",status:"{total} میں سے {from}–{to}",empty:"کوئی ہولڈر نہیں ملا"}
    };
    function copy() {
      var config = window.CalorieTokenSiteStyleMenu;
      ...
      var tag=valid?locale:"en";
      return { text: Object.assign({},valid ? candidate : fallback,pagerCopy[tag]||pagerCopy.en), locale: tag, dir: tag === "ar" || tag === "ur" ? "rtl" : "ltr" };
    }
    ...
    function restoreRows(view) {
      (view.allRows||[]).forEach(function(row){
        if(!view.originalHidden.has(row))return;
        var hidden=view.originalHidden.get(row);if(row.hidden!==hidden)row.hidden=hidden;
      });
      view.allRows=[];
    }
    function tableRows(view) {
      return Array.from(view.table.querySelectorAll("tbody tr")).filter(function(row){
        return !row.closest("form,[contenteditable],.xl-card,[data-calorieapp-embed],[inert],[aria-hidden=\"true\"]");
      });
    }
    function formatStatus(template,values){return template.replace(/\{(from|to|total)\}/g,function(_,key){return String(values[key]);});}
    function paginate(view,translation){
      var rows=tableRows(view),query=String(view.search.value||"").normalize("NFKC").toLocaleLowerCase(translation.locale).trim();
      var filtered=rows.filter(function(row){return !query||String(row.textContent||"").normalize("NFKC").toLocaleLowerCase(translation.locale).indexOf(query)>=0;});
      var pages=Math.max(1,Math.ceil(filtered.length/view.pageSize));view.page=Math.max(0,Math.min(view.page,pages-1));
      var start=view.page*view.pageSize,end=Math.min(start+view.pageSize,filtered.length),visible=new Set(filtered.slice(start,end));
      rows.forEach(function(row){if(!view.originalHidden.has(row))view.originalHidden.set(row,row.hidden);var hidden=view.originalHidden.get(row)||!visible.has(row);if(row.hidden!==hidden)row.hidden=hidden;});
      view.allRows=rows;view.previous.disabled=view.page===0;view.next.disabled=view.page>=pages-1;
      view.search.setAttribute("aria-label",translation.text.search);view.search.placeholder=translation.text.search;
      view.previous.textContent=translation.text.previous;view.next.textContent=translation.text.next;
      view.status.textContent=filtered.length?formatStatus(translation.text.status,{from:start+1,to:end,total:filtered.length}):translation.text.empty;
    }
    function makeView(table) {
      ...
      var controls=document.createElement("div");controls.className="calorieapp-richlist-pager";
      var search=document.createElement("input");search.type="search";search.className="calorieapp-richlist-filter";search.autocomplete="off";search.spellcheck=false;
      var previous=document.createElement("button");previous.type="button";previous.className="calorieapp-richlist-page";
      var status=document.createElement("span");status.className="calorieapp-richlist-page-status";status.setAttribute("role","status");status.setAttribute("aria-live","polite");
      var next=document.createElement("button");next.type="button";next.className="calorieapp-richlist-page";
      controls.append(search,previous,status,next);actions.appendChild(controls);
      region.parentNode.insertBefore(actions, region);
      var view = { table: table, region: region, actions: actions, jump: jump, regionText: regionText, actionText: actionText, rankText: rankText, row: null, search:search, previous:previous, next:next, status:status, page:0, pageSize:100, originalHidden:new WeakMap(), allRows:[] };
      search.addEventListener("input",function(){view.page=0;refresh();});
      previous.addEventListener("click",function(){if(view.page>0){view.page-=1;refresh();region.scrollIntoView({block:"start",behavior:"auto"});}});
      next.addEventListener("click",function(){view.page+=1;refresh();region.scrollIntoView({block:"start",behavior:"auto"});});
      jump.addEventListener("click", function () {
        // Recheck before activation, even before a queued observer delivery.
        refresh();
        if (!view.row || jump.hidden) return;
        view.search.value="";
        var rows=tableRows(view),index=rows.indexOf(view.row);
        if(index>=0)view.page=Math.floor(index/view.pageSize);
        paginate(view,copy());
    ...
    function refresh() {
      ...
      views = views.filter(function (view) {
        ...
        restore(view);restoreRows(view);view.actions.remove(); view.region.removeAttribute("aria-labelledby");
        return false;
      });
      tables.forEach(function (table) {
        if (!views.some(function (view) { return view.table === table; })) views.push(makeView(table));
      });
      var marked = tables.flatMap(function (table) { return Array.from(table.querySelectorAll("tr.xl-is-user")).filter(function(row){return row.isConnected&&!row.closest("form,[contenteditable],.xl-card,[data-calorieapp-embed],[inert],[aria-hidden=\"true\"]");}); });
