...
return /^(?:preview|customize\_changeset\_uuid|brizy-edit|brizy-edit-iframe|brz-edit|brz-edit-iframe)$/.test(key);
});
}
if (!allowed()) return;
...
var requested = new URL(window.location.href).searchParams.get('ui\_lang') || document.documentElement.lang || 'en';
...
"\n\n## Before you start\n\nCalorie (CAL) is a token on the XRP Ledger. Use a supported wallet and check the token’s issuer, not just its name or logo.\n\n**Currency:** Calorie (CAL)  \n**Issuer:** rNqGa93B8ewQP9mUwpwqA19SApbf62U7PY  \n**Currency code:** 43616C6F72696500000000000000000000000000\n\n",
"\n\n## Choose a trading route\n\n**[XPMarket · CAL/XRP order book](\"https://xpmarket.com/dex/Calorie-rNqGa93B8ewQP9mUwpwqA19SApbf62U7PY/XRP\")**  \nOpen the CAL/XRP market directly and review issuer, price, liquidity, fees and price impact before continuing.\n\n**[XPMarket · CAL/XRP swap](\"https://xpmarket.com/swap/Calorie-rNqGa93B8ewQP9mUwpwqA19SApbf62U7PY/XRP/market\")**  \nUse the swap route when available and review the final wallet request before approving it.\n\nXPMarket is an external service. Availability, liquidity, fees and regional access may change. CalorieToken does not execute the trade or custody your wallet.\n\n",
"\n\n## How to buy CAL on the XRP Ledger\n\n1. Set up a supported XRPL wallet using its official website or app. Keep your recovery phrase private.\n2. Add enough XRP for the intended trade and the wallet’s displayed reserve and network fees.\n3. Open one of the XPMarket routes above. Verify Calorie/CAL and the complete issuer address shown here before continuing.\n4. If your wallet requests a CAL trust line, review it before approving. Then enter the amount and inspect the quoted price, liquidity, fees and slippage.\n5. Approve only the transaction you intend in your wallet. Wait for the ledger result and check your resulting balance.\n\nReserve requirements can change; follow your wallet’s current values and the [official XRPL reserve documentation](\"https://xrpl.org/docs/tutorials/best-practices/account-management/calculate-reserves\"). Never enter a recovery phrase on a trading website.\n\n",
"\n\nTrading can result in loss. This page provides practical links and instructions, not an investment recommendation. CAL trading is separate from CalorieApp’s food search and personal food log.\n\n",
].join("");;
function normalizedMarkup(markup) {
var template = document.createElement("template");
template.innerHTML = String(markup || "");
return template.innerHTML.replace(/>\s+<").trim();
}
function blogPanel() {
...
var permitted=false;
try { permitted=typeof window.cmplz\_has\_service\_consent === 'function' && window.cmplz\_has\_service\_consent('twitter') === true; } catch (\_) { /\* Keep the CMP authoritative. \*/ }
...
var consentAvailable=typeof window.cmplz\_has\_service\_consent === 'function'
...
|| Array.from(url.searchParams.keys()).some(function (key) { return key !== "ui\_lang"; })) return;
...
XRPL buying and selling guide

## Buy or sell CAL in four clear steps

CAL trades on the XRP Ledger DEX. Verify the complete issuer and currency code, not just the name or logo.
...
Issuer
:   ```
    rNqGa93B8ewQP9mUwpwqA19SApbf62U7PY
    ```
...
1. 1
   ...
   ### Set up an XRPL wallet

   Install Xaman from its official source. Keep recovery information offline; never enter it on this website or a trading page.

   [Visit Xaman](\"https://xaman.app/\")
",
"2. 2

   ### Fund your wallet with XRP

   Xaman lists independent providers by country and currency. Check eligibility, identity checks, rates and fees; availability varies by region. Already hold BTC, ETH or another supported coin? AllChainBridge / SWFT is another possible route to XRP across networks. See section 3 below. Select the source coin and network, choose XRP on XRP Ledger as the destination, and check the available wallets, route, fees and any destination tag with SWFT. CAL is temporarily unavailable there.

   [Find options in Xaman](\"https://xumm.app/detect/xapp:xumm.buysellxrp\")

   Keep enough XRP for the reserve and network fees shown by your wallet. Reserve requirements can change.

   [XRPL reserve information](\"https://xrpl.org/docs/concepts/accounts/reserves\")
",
"3. 3

   ### Add the exact CAL trustline

   To receive CAL, review its issuer and currency code before approving the trustline in Xaman.

   [Open CAL trustline guide](\"__TRUSTLINE__\")
",
"4. 4

   ### Choose buy or sell

   On XPMarket, check what you pay and receive, the issuers, route, fees, liquidity and price impact. Review the final request in your wallet.

   [Open CAL on XPMarket](\"https://xpmarket.com/dex/Calorie-rNqGa93B8ewQP9mUwpwqA19SApbf62U7PY/XRP\")

   To sell, select CAL as the asset you pay and check the asset you receive. A received token may need its own trustline. Converting proceeds to money in a bank account is a separate provider service.
",
"
",
"

### Trading through external services

This website provides guidance and external links. Trading takes place with the selected external service and your wallet.

",
"**Before you trade**

Trading can cause loss. Project-related wallets may provide liquidity and have a financial interest. Quotes, liquidity and execution are not guaranteed. This guide is not investment advice.

CalorieApp food search and food logging do not require buying CAL.

"
].join("").replace("\_\_TRUSTLINE\_\_", localRoute("trustline"));
...
&& Array.from(url.searchParams.keys()).every(function (key) { return key === "ui\_lang"; });
} catch (\_) { return false; }
}
function source() {
...
if (pageId() === 5580) {
['cmplz\_cookie\_warning\_loaded','cmplz\_status\_change','cmplz\_status\_change\_service','cmplz\_revoke'].forEach(function (name) {
