...
def http_get(path: str, timeout: int = 20) -> tuple[int | None, str]:
    try:
...
def http_post_json(path: str, payload: dict, timeout: int = 20) -> tuple[int | None, dict]:
...
def main() -> int:
    ...
    logs_status, logs_raw = http_get("/logs")
    logs: list[dict] = []
    if logs_status == 200:
        try:
    ...
    logs2_status, logs2_raw = http_get("/logs")
    logs2: list[dict] = []
    if logs2_status == 200:
        try:
