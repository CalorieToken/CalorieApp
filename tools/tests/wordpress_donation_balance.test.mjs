...
function fixture({page=6897, endpoint='https://calorietoken.net/wp-json/calorietoken/v1/donation-balance'}={}) {
  const {document, window: dom} = parseHTML(`<html lang="en"><body class="ctstyle-enabled page-id-${page}"><form><input value="0.01"></form><section id="ctstyle-donation-balance"><h2 data-donation-copy="title"></h2><strong data-donation-amount>—</strong><p data-donation-status></p><time data-donation-time></time><span data-donation-ledger></span></section></body></html>`);
  let now=1800000000000, seq=0, calls=0, requests=[], response={wallet,network:'mainnet',balanceDrops:'12345678912345678',ledgerIndex:100,checkedAt:now/1000,validated:true,status:'current'};
  ...
  return {document,window,timers,requests,flush,get calls(){return calls;},
    ...
    setResponse(r){response=r;},good(overrides={}){return {wallet,network:'mainnet',balanceDrops:'12345678',ledgerIndex:101,checkedAt:now/1000,validated:true,status:'current',...overrides};},
    async emit(n){for(const fn of events.get(n)||[])fn();await flush();}
  };
}
test('Balance retains every drop, uses one same-origin read and preserves donation controls',async()=>{
  const h=fixture(),input=h.document.querySelector('input');await h.run();
...
test('Timeouts or wrong-wallet responses retain last measurement and explicitly show stale status',async()=>{
  ...
  assert.equal(h.document.querySelector('[data-donation-amount]').textContent,'0.00');
