...
from tools import build_heading_repair_release as release


class HeadingRepairReleaseTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_repeatable_archive_contains_exact_runtime_sources(self):
        ...
        with zipfile.ZipFile(self.root / "first" / first["archive"]) as archive:
            ...
            for path in release.FILES:
                self.assertEqual(
                    archive.read(f"{release.SLUG}/{path}"),
                    (self.plugin / path).read_bytes(),
                )
        manifest = json.loads(
            (self.root / "first" / f"{release.SLUG}-{first['version']}.manifest.json").read_text()
        )
    ...
    def test_missing_asset_is_rejected(self):
        (self.plugin / "assets" / "nutrition-summary.js").unlink()
        with self.assertRaisesRegex(ValueError, "inventory"):
            release.build(self.root / "output", self.plugin)

    def test_version_mismatch_is_rejected(self):
        main = self.plugin / f"{release.SLUG}.php"
        main.write_text(main.read_text().replace("const VERSION = '", "const VERSION = '99"))
        with self.assertRaisesRegex(ValueError, "version"):
            release.build(self.root / "output", self.plugin)

    def test_symlink_is_rejected(self):
        asset = self.plugin / "assets" / "nutrition-summary.js"
        asset.unlink()
        asset.symlink_to(release.PLUGIN / "assets" / "nutrition-summary.js")
