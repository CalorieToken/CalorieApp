...
the final comparison. A malformed or non-string expected digest returns
`False`; invalid secret, receipt, identifier, channel or timeline audit input
raises `ValueError` under the builder's existing validation rules. HMAC input
is fed incrementally to avoid allocating one combined domain-plus-payload buffer.
