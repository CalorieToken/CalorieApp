...
| Zero-additional-cost operation | One Neon Free PostgreSQL 16 project exists in Frankfurt for isolated synthetic staging; billing is $0, no payment method or API key exists, DPA execution, subprocessor notifications, native hard limits, offline encryption custody and the protected synthetic workflow are confirmed, and no application data operation has run | Run the single separately approved synthetic migration, process replacement, restart-window reconnect, encrypted backup and exit restore before any real data |
